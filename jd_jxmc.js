...
function shareCodesFormat () {
  return new Promise( async resolve => {
    ...
    if ( readShareCodeRes ) {
      $.newShareCodes = [ ...new Set( [ ...$.inviteCodeList, ...( $.res || [] ), ...( readShareCodeRes || [] ) ] ) ];
    } else {
...
function readShareCode () {
  return new Promise( async resolve => {
    $.get( { url: `https://cdn.jsdelivr.net/gh/zhaozhanzhan/jsdelivrCDN@main/shareCodes/jxmc.json`, timeout: 30 * 1000 }, ( err, resp, data ) => {
      try {
        if ( err ) {
          console.log( JSON.stringify( err ) )
